...
const ProjectExplorer = ({ username, userId, token, appName, onFileClick }) => {
  ...
  const fetchDirectoryStructure = async () => {
    ...
    try {
      const response = await axios.get(
        `${process.env.REACT_APP_API_URL}/api/apps/directory`,
        {
          headers: { "x-auth-token": token },
          params: { userId, path: `/home/user/${username}/${username}-${appName}` },
        }
      );
      ...
      const formatData = (node) => ({
        ...
        ...node.children.reduce((acc, child) => ({ ...acc, ...formatData(child) }), {}),
      });
  ...
  const handleContextMenu = (event, item) => {
    ...
    setSelectedPath(item ? item.index : null);
