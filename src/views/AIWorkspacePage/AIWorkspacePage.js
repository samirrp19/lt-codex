...
import WorkspaceStore from "./components/WorkspaceStore";
import WorkspaceHeader from "./components/WorkspaceHeader";
import ChatPromptContainer from "./ChatPromptContainer";
...
const AIWorkspacePage = () => {
  ...
  const [selectedSidebarTab, setSelectedSidebarTab] = useState("prompt");
        ...
        <Box sx={{
          ...
          <Box sx={{ display: "flex", flex: 1, overflow: "hidden", marginTop: "110px" }}>
            <Box sx={{ flex: 1, display: "flex", flexDirection: "column", minHeight: 0 }}>
              {selectedSidebarTab === "prompt" && activeWorkspace && (
                <ChatPromptContainer
                  postType={activeWorkspace?.postType || 'template'}
                  appId={projectId}
                  ...
                  selectedLanguage={activeWorkspace?.framework}
                  podcastEnabled={podcastEnabled}
                  documentationEnabled={documentationEnabled}
                />
              )}
              
              {selectedSidebarTab === "members" && (
                <WorkspaceMembers />
              )}
              {selectedSidebarTab === "media" && (
                <WorkspaceStore workspaceId={templateId}/>
              )}
            </Box>
          </Box>
        </Box>
