...
const PromptsPanel = ({ appId, workspaceId, efsPath }) => {
  ...
  const currentFileTitle = useRef(null);
      ...
      if (data.history) {
        const formattedHistory = data.history.flatMap((item) =>
          item.responses.map((response) => ({
            ...
            files: Object.entries(response.files || {}).map(([filePath, fileData]) => ({
              file_path: filePath,
              content: fileData.content,
              install: fileData.install || [],
  ...
  useEffect(() => {
    ...
    socket.on("streamed_response", (data) => {
      ...
      const title = incoming.title || currentFileTitle.current;
      if (!title) return;

      currentFileTitle.current = title;

      const existing = partialResponseRef.current[title] || {
        title,
        summary: "",
        documentation: "",
        code: "",
      ...
      if (incoming.code && !existing.code.endsWith(incoming.code.trim())) {
      ...
      partialResponseRef.current[title] = existing;
                  ...
                  <>
                    {res.title && (
                      <Typography variant="h6" sx={{ color: "#4a90e2", fontWeight: "bold", mb: 1 }}>
                        <ReactMarkdown>{res.title}</ReactMarkdown>
                      </Typography>
                    )}
                    {res.summary && (
                    ...
                    {res.code && res.install && res.install.length > 0 && (
