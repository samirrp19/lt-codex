...
import UploadModal from './UploadModal'; // Assuming UploadModal is created with MUI
import NewFileModal from './NewFileModal'; // Assuming NewFileModal is created with MUI
import StoreAlert from './StoreAlert'; // Assuming StoreAlert is for alert handling

const DocStoreContainer = ({ files: initialFiles = [], token, username }) => {
  ...
  const [showNewFileModal, setShowNewFileModal] = useState(false); // State for New File modal
    ...
    <Box sx={{ p: 3 }}>
      ...
      <Box display="flex" justifyContent="space-between" alignItems="center" mb={3}>
        <Box sx={{ display: 'flex', gap: 2 }}>
          {/* Create Button with hover visibility */}
          <Button
            startIcon={<CreateNewFolderIcon />}
            onClick={() => setShowNewFileModal(true)} // Open New File Modal
            sx={{
              opacity: 0.6,
              '&:hover': {
          ...
          {/* Upload Button with hover visibility */}
          <Button
